# CryoBrain — Spec v4 (demo-ready)

**The NPU brain inside the quantum chip that keeps it alive.** A verifiable RL
environment that drives real-time neural QEC decoder **design optimization** — graded on
...
> Honest-claim note: the system *optimizes designs against a verifiable reward*. Say
> "optimization driven by a verifiable reward," not "the AI authors RTL from scratch"
> (that's only true once the Fireworks GRPO path is real). The demo and all copy obey this.
...
## 10. Demo — a LIVE DASHBOARD, not slides

**The demo is one live screen driven by real artifact data.** Full build brief:
`DASHBOARD_SPEC.md`. Summary:

- **One self-contained file, runs offline, 2×2 panels**, every number traced to a real
  artifact (`source: artifacts/<file>.json` captions = honesty + credibility flex).
- **Panels:** A waveform (`cryo_golden_trace.vcd`, syndrome→correction), B climb
  (`climb_chart_rl.json`, animated 0.364→0.459), C memory A/B
  (`memory_ab_overlay.json`, **headline the ~2× slope, not the endpoint**), D Pareto
  (`designs.json` + **real Yosys area**, budget box, MWPM off-budget, winner green).
- **"▶ Play story"** runs the beats: gate-honesty (wrong→0) → climb → memory (2× faster)
  → Pareto verdict → hold for the business close.
- **Built with design tooling** (Claude frontend-design / Claude Design or Grok) — must
  look intentional, instrument-panel aesthetic, not a default dashboard.

**Framing rules (hard):** "optimization driven by a verifiable reward," NOT "AI authors
RTL"; accuracy shown **continuous** (LER), never 100%/72%; memory framed as **slope**;
concept art (Gizmo/architecture) is a *separate* bookend tagged "concept", never inside
the four data panels. Record a screen-capture backup; present live, cut to tape if it stalls.

If training stalls: drop the climb/memory panels' RL framing, demo the env + validity gate
+ one real design point + classical fallback. Still a complete story.
...
- [ ] **Live dashboard** (`DASHBOARD_SPEC.md`) renders all four panels from real artifacts, "Play story" works, screen-capture backup recorded.
- [ ] Beachhead+moonshot business close on the dashboard footer; one checkable customer claim.
